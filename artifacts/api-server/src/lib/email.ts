...
export async function sendVerificationEmail(email: string, code: string, username: string = "there"): Promise<void> {
  ...
  const html = emailBase(`
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td style="padding:36px 44px 28px 44px;border-bottom:1px solid #27272a;">
        <table cellpadding="0" cellspacing="0"><tr>
          <td style="background:#2d1b69;border:1px solid #4c1d95;border-radius:10px;padding:8px 14px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td style="padding:0 44px 28px 44px;border-top:1px solid #27272a;">
        <p style="margin:0;padding-top:20px;font-size:12px;color:#52525b;line-height:1.7;">
          If you didn't create a Hex Auth account, ignore this email.<br/>
...
export async function sendPasswordResetEmail(email: string, code: string, username: string = "there"): Promise<void> {
  const from = process.env.SMTP_FROM ?? "Hex Auth <noreply@benjahexauth.qzz.io>";

  if (!process.env.SMTP_PASS) {
    logger.info({ email, code }, "Password reset code (SMTP not configured)");
    return;
  }
  ...
  const html = emailBase(`
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td style="padding:36px 44px 28px 44px;border-bottom:1px solid #27272a;">
        <table cellpadding="0" cellspacing="0"><tr>
          <td style="background:#1a1530;border:1px solid #3b2f6e;border-radius:10px;padding:8px 14px;">
            <span style="font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#a78bfa;">Password Reset</span>
          </td>
        </tr></table>
        <h1 style="margin:14px 0 8px 0;font-size:24px;font-weight:700;color:#f4f4f5;line-height:1.3;">Reset your password</h1>
        <p style="margin:0;font-size:15px;color:#71717a;line-height:1.6;">
          Hi <strong style="color:#e4e4e7;">${username}</strong> — use the code below to reset your Hex Auth password. This code expires in 15 minutes.
        </p>
      </td>
    </tr></table>
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td style="padding:32px 44px;">
        <table width="100%" cellpadding="0" cellspacing="0"><tr>
          <td style="background:#0f0f13;border:1px solid #3f3f46;border-radius:14px;padding:28px;text-align:center;">
            <p style="margin:0 0 12px 0;font-size:11px;font-weight:600;letter-spacing:0.14em;text-transform:uppercase;color:#52525b;">Password Reset Code</p>
            <p style="margin:0 0 10px 0;font-size:48px;font-weight:800;letter-spacing:0.22em;color:#a78bfa;font-family:'Courier New',Courier,monospace;">${code}</p>
            <table cellpadding="0" cellspacing="0" style="margin:0 auto;"><tr>
              <td style="background:#1c1917;border:1px solid #292524;border-radius:20px;padding:5px 14px;">
                <span style="font-size:12px;color:#78716c;">⏱ Expires in <strong style="color:#a8a29e;">15 minutes</strong></span>
    ...
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td style="padding:0 44px 12px 44px;">
        <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #27272a;border-radius:12px;overflow:hidden;">
          <tr><td style="padding:14px 18px;border-bottom:1px solid #27272a;">
            <span style="font-size:13px;color:#71717a;">🔒 Never share this code. <strong style="color:#a1a1aa;">Hex Auth will never ask for it over chat.</strong></span>
          </td></tr>
          <tr><td style="padding:14px 18px;">
            <span style="font-size:13px;color:#71717a;">🛡️ If you did not request this, your password is safe — just ignore this email.</span>
          </td></tr>
        </table>
      </td>
    </tr></table>
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td style="padding:20px 44px 32px 44px;border-top:1px solid #27272a;">
        <p style="margin:0;padding-top:0;font-size:12px;color:#52525b;line-height:1.7;">
          This reset was requested for <a href="mailto:${email}" style="color:#7c3aed;text-decoration:none;">${email}</a>.
        </p>
      </td>
    </tr></table>
  `);

  const subject = `Reset your Hex Auth password — code: ${code}`;
  const text = `Hi ${username},\n\nYour Hex Auth password reset code is: ${code}\n\nThis code expires in 15 minutes. Do not share it with anyone.\n\nIf you didn't request this, ignore this email.\n\n-- Hex Auth`;
  ...
  try {
    ...
    logger.error({ err: err?.message ?? err }, "Failed to send password reset email");
    throw err;
  }
}

export async function sendTeamInviteEmail(email: string, inviterUsername: string, role: string, acceptLink: string): Promise<void> {
  const from = process.env.SMTP_FROM ?? "Hex Auth <noreply@benjahexauth.qzz.io>";
  ...
  const roleDesc = role === "admin" ? "Full access to create, edit, and manage all resources." : "Read-only access to view all dashboard data.";
  ...
  const html = emailBase(`
    ...
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td style="padding:32px 44px 28px 44px;">
        ...
        <table cellpadding="0" cellspacing="0" style="margin-bottom:28px;width:100%;"><tr>
          <td align="center">
            <a href="${acceptLink}" style="display:inline-block;background:linear-gradient(135deg,#7c3aed,#4f46e5);color:#ffffff;text-decoration:none;padding:16px 48px;border-radius:12px;font-weight:700;font-size:16px;box-shadow:0 4px 24px rgba(124,58,237,0.4);">
              ✓ &nbsp;Accept Invitation
            </a>
          </td>
        </tr></table>
        <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #27272a;border-radius:12px;overflow:hidden;">
          <tr><td style="padding:14px 18px;border-bottom:1px solid #27272a;">
            <span style="font-size:13px;color:#71717a;">👤 You need a <strong style="color:#a1a1aa;">Hex Auth account</strong> to accept. <a href="${acceptLink.split("/api/")[0]}/register" style="color:#8b5cf6;text-decoration:none;font-weight:600;">Register here</a> if needed.</span>
          </td></tr>
          <tr><td style="padding:14px 18px;">
            <span style="font-size:13px;color:#71717a;">⏳ This invitation expires in <strong style="color:#a1a1aa;">48 hours</strong>.</span>
          </td></tr>
        </table>
      </td>
    </tr></table>
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td style="padding:0 44px 28px 44px;border-top:1px solid #27272a;">
        <p style="margin:0;padding-top:20px;font-size:12px;color:#52525b;line-height:1.7;">
          If you were not expecting this invitation, ignore it. No changes will be made.<br/>
  ...
  const text = `Hi,\n\n${inviterUsername} invited you to join their Hex Auth workspace as ${roleLabel}.\n\nAccept here: ${acceptLink}\n\nExpires in 48 hours.\n\n-- Hex Auth`;

  try {
    const transporter = createTransporter();
    await transporter.verify();
    const info = await transporter.sendMail({ from, to: email, subject: `${inviterUsername} invited you to join their Hex Auth team`, text, html });
    logger.info({ email, messageId: info.messageId }, "Team invite email sent");
  } catch (err: any) {
