...
router.get("/blacklist", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  const entries = await Blacklist.find({ ownerId: new Types.ObjectId(req.userId) });
...
router.post("/blacklist", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  const entry = await Blacklist.create({ ownerId: new Types.ObjectId(req.userId), type: parsed.data.type, value: parsed.data.value, reason: parsed.data.reason ?? null });
...
router.delete("/blacklist/:id", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  const entry = await Blacklist.findOneAndDelete({ _id: req.params.id, ownerId: new Types.ObjectId(req.userId) });
