...
router.get("/sessions", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  const sessions = await Session.find({ ownerId: new Types.ObjectId(req.workspaceId!) });
...
router.delete("/sessions/:id", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  const session = await Session.findOneAndDelete({ _id: req.params.id, ownerId: new Types.ObjectId(req.workspaceId!) });
